...
def save_embedding(hotel_name:str) -> pd.DataFrame:
    """
    Saves the embedding
    ...
    path = f"../data/pdf/{hotel_name}.pdf"
    
    df = get_df_embedding(path)
    
    df.to_parquet(f'../data/embeddings/{hotel_name}.parquet')
    
    return df


def load_embedding(hotel_name:str) -> pd.DataFrame:
    """
    ...
    """
    if not os.path.exists(f'../data/embeddings/{hotel_name}.parquet'):
        raise FileNotFoundError(f'Load embedding trying to open not existing file: "{hotel_name}.parquet"')

    df = pd.read_parquet(f'../data/embeddings/{hotel_name}.parquet')
    
    return df


def load_save_embedding(hotel_name: str, force_saving=False) -> pd.DataFrame:
    """
    ...
    Returns:
        pd.Dataframe: dataframe with two cols, text and embeddings
    """
    path_embedding = f'../data/embeddings/{hotel_name}.parquet'
    
    if os.path.exists(path_embedding) and not force_saving:
        print("\n\n\n LOADANDO")
        return load_embedding(hotel_name)
    else:
        print("\n\n\n SALVANDO")
        return save_embedding(hotel_name)


def run_embeddings(prompt:str, hotel_name:str, additional_context={}, force_saving=False) -> tuple[str, list]:
    """
    ...
    """
    df = load_save_embedding(hotel_name, force_saving)
